...
export function wallLevelElevation_mm(
  ...
  if (!levelId) return 0
  return levelElevation_mm(scene, levelId)
}
...
export function levelElevation_mm(
  ...
  for (const sib of siblings) {
    ...
    cumulative += levelHeight_mm(scene, sib.id)
  }
  return cumulative
}

/** Returns the floor-to-floor height of a single level, in mm. */
export function levelHeight_mm(scene: SceneLike, levelId: string): number {
  ...
  for (const childId of level.children) {
    ...
    const h = (child.height ?? 0) * 1000
    if (h > maxTopMm) maxTopMm = h
  }
