...
export const CFSMember = z
  .object({
    ...
    // Pascal cascade-delete walks `parent.children`. Service holes
    // (Slice 6) get parentId = member, and Pascal's `createNode` will
    // auto-append their ids here as long as this field exists. Without
    // it, service holes outlive their member when the wall is deleted.
    // Default `[]` keeps existing scene JSON backward-compatible.
