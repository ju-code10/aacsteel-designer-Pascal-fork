...
describe('runFramingPass — L/T corner lap (first-placed runs through)', () => {
  ...
  it('L-CORNER: first-placed wall A is unchanged; second-placed wall B is shortened by A web depth', async () => {
    ...
    // depth (~92 mm for 362S162-54) inside B's wall, not at B's start —
    // full web so the butt chord sits clear of the through-wall chord's bbox.
    ...
    expect(extB!.startX).toBeLessThan(110)
  ...
  it('L-CORNER: B chord stud sits at the trimmed start, not at the architectural corner', async () => {
    ...
    // B's start-chord world position: should be at (3000, ~92, 0) — one
    // 362-series web depth INSIDE B's wall from the corner (3000, 0).
    ...
    expect(buttChord.z).toBeLessThan(110)
  })
  ...
  it('L-CORNER: swapped placement order swaps which wall trims', async () => {
    ...
    // East wall butts at its END; trimmed by the through wall's full web (~92 mm).
    expect(eastExt!.endX).toBeGreaterThan(2880)
    expect(eastExt!.endX).toBeLessThan(2920)
