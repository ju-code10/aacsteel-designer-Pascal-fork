...
/**
 * Default chord open direction at a *free* end (no perpendicular peer):
 * the cross-section's mouth faces along the wall axis toward the wall's
 * body, matching the field-stud convention. For the START chord that's
 * +wallDir; for the END chord that's -wallDir.
 */
function defaultChordOpenDirection(
  wallDir: { x: number; z: number },
  isStart: boolean,
): { x: number; z: number } {
  const s = isStart ? 1 : -1
  return { x: wallDir.x * s, z: wallDir.z * s }
}
 ...
 * plane, which in world coordinates is `mesh_X_world * cos + mesh_Y_world
 * * sin`. Solving for `orient`:
...
function chordOrientationDeg(
  ...
  // Clamp to [-180, 180] to match the schema's bounds.
...
function buildDesiredMembers(
  ...
  //
  // Orientation: at L/T corners the chord's C-section is rotated so its
  // open mouth faces the perpendicular peer wall's body (= building
  // interior across the corner). At free ends the C opens along the wall
  // axis into the wall body (matches field-stud orientation).
  ...
  for (const [x, owns, promoteToKing, isStart, junction] of [
    ...
    const openDir = junction.peerBodyDirection
      ?? defaultChordOpenDirection(wallDir, isStart)
    const orientation_deg = chordOrientationDeg(wallDir, openDir)
    desired.push({
      role,
